...
//import { CreditCardIcon, CheckCircleIcon, ArrowLeftIcon, XCircleIcon } from '@heroicons/vue/outline';
...
const realizarCompra = async () => {
  ...
  try {
    ...
    console.error(err);
    error.value = 'Error al procesar el pago. Por favor intenta nuevamente.';
  } finally {
            ...
            <div v-for="key in keys" :key="key.id" class="p-6 border-b border-gray-700 last:border-b-0">
              <div class="flex flex-col sm:flex-row gap-6">
                <div class="flex-shrink-0">
                  <img 
                    :src="key.game?.img || '/placeholder-game.jpg'" 
                    :alt="key.game?.name" 
                    class="w-32 h-32 object-cover rounded-lg"
                  >
                </div>
                <div class="flex-grow">
                  <h3 class="text-lg font-semibold mb-2">{{ key.game?.name }}</h3>
                ...
                <div class="flex-shrink-0 text-right">
                  <p class="text-lg font-bold text-yellow-400">${{ key.price }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gray-800 rounded-xl shadow-lg p-6 sticky top-6">
            ...
            <div class="space-y-4">
              <div class="flex justify-between">
                ...
                <span>${{ keys.reduce((sum, k) => sum + Number(k.price), 0) }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-400">Impuestos</span>
                <span>${{ keys.reduce((sum, k) => sum + Number(k.tax || 0), 0) }}</span>
              </div>
              <div class="border-t border-gray-700 pt-4 flex justify-between font-bold text-lg">
                <span>Total</span>
                <span class="text-yellow-400">${{ keys.reduce((sum, k) => sum + Number(k.price) + Number(k.tax || 0), 0) }}</span>
              </div>
            </div>
            ...
            <button
              ...
              :class="{ 'opacity-70 cursor-not-allowed': processingPayment }"
            >
              <span v-if="!processingPayment">Confirmar y pagar</span>
            ...
            <p class="text-xs text-gray-500 mt-4 text-center">
              Al completar tu compra, aceptas nuestros <a href="#" class="text-yellow-400 hover:underline">Términos de servicio</a> y <a href="#" class="text-yellow-400 hover:underline">Política de privacidad</a>.
            </p>
          </div>
        </div>
      </div>
