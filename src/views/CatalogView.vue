...
// Obtener game keys disponibles
const fetchGameKeys = async () => {
  try {
    isLoading.value = true;
    // Simulamos la respuesta con los datos proporcionados
    ...
    // Extraer plataformas y regiones únicas para los filtros
...
// Extraer plataformas, regiones y precios únicos de los datos
const extractUniqueFilters = () => {
  ...
  gameKeys.value.forEach(key => {
    ...
    // Convertir el precio a número y agregarlo al array
  ...
  // Calcular mínimo y máximo de precios
  if (prices.length > 0) {
...
const applyFilters = () => {
  ...
  // Filtro por rango de precio (convertir a número)
  results = results.filter(key => {
    const price = parseFloat(key.price);
...
// Reaccionar a cambios en los filtros o query
watch(
  () => ({
    query: route.query.q,
    ...filters.value
...
// Watcher para mantener el rango de precios válido
watch(
  ...
  (newRange) => {
    ...
    // Asegurar que no excedan los límites originales
  ...
  <section class="bg-gray-900 text-white flex flex-col min-h-screen">
      ...
      <div class="flex justify-center">
        <div class="relative w-full max-w-2xl">
          <input 
            ...
            class="w-full p-2 pl-10 rounded-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-yellow-500"
          >
          <svg 
    ...
    <section class="container mx-auto p-6 flex-grow flex flex-col md:flex-row">
      ...
      <aside class="w-full md:w-64 bg-gray-800 p-4 rounded-lg md:mr-6 mb-6 md:mb-0">
        ...
        <!-- Filtro por precio -->
        <div class="mb-6">
          <h3 class="font-semibold mb-2">Rango de precio (${{ filters.priceRange[0] }} - ${{ filters.priceRange[1] }})</h3>
          <div class="flex space-x-4 mb-2">
            <input 
              type="number" 
              v-model="filters.priceRange[0]"
              :min="filters.originalPriceRange[0]"
            ...
            <input 
              ...
              :max="filters.originalPriceRange[1]"
              @change="applyFilters"
              class="w-1/2 p-1 rounded bg-gray-700 text-white border border-gray-600 focus:border-yellow-500 focus:ring-yellow-500"
            >
          </div>
