      ...
      <div class="flex flex-col md:flex-row gap-8 mb-8">
        ...
        <div class="bg-gray-800 p-6 rounded-lg w-full md:w-1/3">
          <div class="flex flex-col items-center">
            <img 
              :src="user.avatar ? `${user.avatar}` : '../img/def.jpg'" 
              class="w-32 h-32 rounded-full object-cover mb-4 border-2 border-yellow-500"
            />
            <h2 class="text-2xl font-bold">{{ user.username }}</h2>
            <p class="text-gray-400">{{ user.email }}</p>
            
            <div class="mt-4 px-4 py-2 rounded-full text-sm font-semibold" 
                 :class="{
        ...
        <div class="bg-gray-800 p-6 rounded-lg w-full md:w-2/3">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input 
                type="text" 
                :value="new Date(user.register_date).toLocaleDateString()" 
                class="w-full p-2 rounded bg-gray-700 text-white" 
                disabled
              />
            </div>
