...
<section class="bg-gray-900 text-white flex flex-col min-h-screen">
    ...
    <main class="container mx-auto p-6 flex-grow">
        <button @click="action = 'add'; name = ''; id_genre = '';" class="text-sm flex-1 bg-yellow-500 text-gray-900 px-4 py-2 rounded-lg hover:bg-yellow-400">Agregar</button>
            ...
            <div class="bg-gray-800 p-6 rounded-lg">
                ...
                <ul class="space-y-2">
                    <div v-for="genre in genres" :key="genre.id">
                        <li>{{ genre.name }}</li>
                        <button @click="selectGenre(genre.id)" class="text-center text-sm bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-400">Editar</button>
                        <button @click="deleteGenre(genre.id)" class="text-sm flex-1 bg-yellow-500 text-gray-900 px-4 py-2 rounded-lg hover:bg-yellow-400">Eliminar</button>
                    </div>
                </ul>
            </div>
        </div>
    ...
    <div>
        <form @submit.prevent="action === 'add' ? addGenre() : editGenre()" class="bg-gray-800 p-6 rounded-lg">
